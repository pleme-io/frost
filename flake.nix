  ...
  outputs = { nixpkgs, flake-utils, substrate, devenv, ... }:
    ...
    in {
      packages = {
        ...
        # blfrost — standalone frost shell wrapper (analogous to blzsh)
        # Launches frost with an optional FROST_DIR config directory.
      ...
      apps = {
        ...
        # nix run .#verify — verify shell config load order
        verify = mkApp "verify" ''
          set -euo pipefail
